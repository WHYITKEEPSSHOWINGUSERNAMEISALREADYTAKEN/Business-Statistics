...
### Summarizing data
##  Pardoe, Chapter 1: 1.1

HOMES1 <- read.table("d1802_HOMES1.txt",header=TRUE)   # Read data for analysis
attach(HOMES1)
Y
stemY <- stem(Y,scale=0.5) #Stem-and-Leaf, illustrated on p.2. where argument "scale" controls the plot length
histY <- hist(Y, freq = FALSE, breaks = c(150,175,200,225,250,275,300,325,350,375,400), 
         ylab = "Frequency", xlab = "Y(price in $ thousands)")   # Figure1.1., illustrated on p.3
boxplot(Y)   # boxplot, suggested on p.4
...
pairs(airpoll, panel=function(x,y) {abline(lsfit(x,y)$coef, lwd=1);
#                                  lines(lowess(x,y), lty=2, lwd=1);
                                   points(x,y)})
...
HOMES2 <- read.table("d1802_HOMES2.txt",header=TRUE) #Read data for analysis
attach(HOMES2)
fit <- lm(Y~X)
summary(fit)
